...
def read_data(root_dir):
    ...
    return x_train, y_train, y_train_non_scored, x_test
...
def add_cluster_gene_cell_features(df_features_all, n_clusters_g, n_clusters_c, base_seed, is_train, out_data_dir):
    ...
    def create_cluster(data_all, features, kind='g', n_clusters=n_clusters_g):
        ...
        if is_train:
            kmeans = KMeans(n_clusters=n_clusters, random_state=base_seed).fit(data)
            pickle.dump(kmeans, open(os.path.join(out_data_dir, f'kmeans_{kind}.pkl'), 'wb'))
        else:
            kmeans = pickle.load(open(os.path.join(out_data_dir, f'kmeans_{kind}.pkl'), 'rb'))

        data_all[f'clusters_{kind}'] = kmeans.predict(data.values)
        data_all = pd.get_dummies(data_all, columns=[f'clusters_{kind}'])
        return data_all

    df_features_all = create_cluster(df_features_all, gene_cols, kind='g', n_clusters=n_clusters_g)
    df_features_all = create_cluster(df_features_all, cell_cols, kind='c', n_clusters=n_clusters_c)
    print(f"data shape after cluster feature: {df_features_all.shape}")
    return df_features_all


def add_cluster_pca_features(df_features_all, n_comp_cluster_pca, base_seed, is_train, out_data_dir):
    gene_pca_cols = [col for col in df_features_all.columns if col.startswith('pca_g')]
    cell_pca_cols = [col for col in df_features_all.columns if col.startswith('pca_c')]

    print("Cluster PCA")
    data_pca = pd.concat((df_features_all[gene_pca_cols], df_features_all[cell_pca_cols]), axis=1)
    if is_train:
        kmeans_pca = KMeans(n_clusters=n_comp_cluster_pca, random_state=base_seed).fit(data_pca)
        pickle.dump(kmeans_pca, open(os.path.join(out_data_dir, 'kmeans_pca.pkl'), 'wb'))
    else:
        kmeans_pca = pickle.load(open(os.path.join(out_data_dir, 'kmeans_pca.pkl'), 'rb'))

    df_features_all[f'cluster_pca'] = kmeans_pca.predict(data_pca.values)
    data_all = pd.get_dummies(df_features_all, columns=[f'cluster_pca'])
...
class PytorchPreprocessHelper:
    ...
    def process(self, preprocess_param, base_seed):
        ...
        if preprocess_param['is_add_cluster_gene_cell']:
            df_feature_all = add_cluster_gene_cell_features(df_feature_all, preprocess_param['n_clusters_g'],
                                                            preprocess_param['n_clusters_c'], base_seed,
                                                            is_train=self.is_train,
                                                            out_data_dir=self.out_pytorch_data_dir)

        if preprocess_param['is_add_cluster_pca']:
            df_feature_all = add_cluster_pca_features(df_feature_all, preprocess_param['n_comp_cluster_pca'], base_seed,
                                                      is_train=self.is_train, out_data_dir=self.out_pytorch_data_dir)
        ...
        self.y_train = self.df_y_train
        self.y_train_non_scored = self.df_y_train_with_non_scored

        return self.x_train, self.y_train, self.y_train_non_scored, self.x_test, self.x_train.columns, self.y_train.columns
